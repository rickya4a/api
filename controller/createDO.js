...
exports.getDataHeader = (req, res) => {
    ...
    pool_whm.then(pool => {
      pool.request()
      .input('initiateId', sql.VarChar, req.params.initiate_do_id)
      .query(`SELECT b.NAMA_STOCKIES,a.ID_STOCKIES, b. ALAMAT_STOCKIES,a.ID_WAREHOUSE, 
...
exports.getDetailProduct = (req, res) => {
  ...
  pool_whm.then(pool => {
    ...
    FROM klink_whm_testing.dbo.T_DETAIL_INITIATE_DO a
      LEFT join klink_whm_testing.dbo.T_SALESSIMULATION b ON a.ID_KWITANSI=b.KWITANSI_NO
      LEFT join klink_whm_testing.dbo.MASTER_PRODUK_ALIAS c ON b.PRODUK_ALIAS_ID=c.PRODUK_ALIAS_ID
      LEFT JOIN klink_whm_testing.dbo.MASTER_PRODUK E ON E.ID_PRODUCT = c.ID_PRODUCT
...
exports.getMasterProduct = (req, res) => {
  ...
  pool_whm.then(pool => {
    pool.request()
    .input('productId', sql.VarChar, req.params.id_product)
...
exports.checkBox = (req, res) => {
  ...
  pool_whm.then(pool => {
    pool.request()
    .input('productId', sql.VarChar, req.params.id_product)
    .input('warehouseId', sql.VarChar, req.params.id_warehouse)
...
exports.processReceiptNo = (req, res) => {
  ...
  pool_whm.then(pool => {
    ...
    FROM klink_whm_testing.dbo.T_DETAIL_INITIATE_DO A
    INNER JOIN klink_whm_testing.dbo.T_INITIATE_DO B ON A.INITIATE_DO_ID = B.INITIATE_DO_ID
    WHERE A.INITIATE_DO_ID = @initiateId AND B.STATUS != '1'`, (err, result) => {
      if (err) throw err
      res.json(result.recordset)
    })
  })
...
exports.updateFlagInitiate = (req, res) => {
  ...
  pool_whm.then(pool => {
    pool.request()
    .input('initiateId', sql.VarChar, req.params.initiate_do_id)
    .query(`UPDATE klink_whm_testing.dbo.T_INITIATE_DO SET STATUS = '1' WHERE INITIATE_DO_ID = @initiateId`, (err, result) => {
      if (err) throw err
...
exports.processProductSingle = (req, res) => {
  ...
  pool_whm.then(pool => {
    ...
    .query(`SELECT A.KWITANSI_NO , B.ID_PRODUCT, A.QTY_SISA AS QTY, A.PRODUK_ALIAS_ID, A.IS_INDENT
    ...
    WHERE A.KWITANSI_NO = @receiptNo AND A.QTY_SISA > 0 AND A.IS_BUNDLED != 1`, (err, result) => {
      if (err) throw err
      res.json(result.recordset)
    })
  })
...
exports.upFlagIndentSales = (req, res) => {
  ...
  pool_whm.then(pool => {
    pool.request()
    .input('receiptNo', sql.VarChar, req.params.kw)
    .input('aliasId', sql.VarChar, req.params.produk_alias_id)
    .query(`UPDATE klink_whm_testing.dbo.T_SALESSIMULATION SET IS_INDENT = 0  WHERE KWITANSI_NO = @receiptNo AND PRODUK_ALIAS_ID = @aliasId`, (err, result) => {
      if (err) throw err
      res.json(result.recordset)
    })
  })
...
exports.updateFlagIndent = (req, res) => {
  ...
  pool_whm.then(pool => {
    ...
    .input('receiptNo', sql.VarChar, req.params.kw)
    .input('aliasId', sql.VarChar, req.params.produk_alias_id)
    .query(`UPDATE klink_whm_testing.dbo.T_INDENT SET IS_ACTIVE = 1   WHERE ID_KWITANSI = @receiptNo AND PRODUK_ALIAS_ID = @aliasId`, (err, result) => {
      if (err) throw err
      res.json(result.recordset)
    })
  })
...
exports.checkStockProductBox = (req, res) => {
  ...
  pool_whm.then(pool => {
    ...
    .input('productId', sql.VarChar, req.params.id_product)
    .input('warehouseId', sql.VarChar, req.params.id_warehouse)
    .query(`SELECT A.ID_RACK, A.ID_PRODUCT, A.QTY, A.EXP_PABRIK, A.EXP_GUDANG, C.PRODUCT_NAME, D.WAREHOUSE_NAME
    FROM klink_whm_testing.dbo.T_STOCK A
    LEFT JOIN klink_whm_testing.dbo.MASTER_RACK B ON A.ID_RACK = B.ID_RACK
    LEFT JOIN klink_whm_testing.dbo.MASTER_PRODUK C ON A.ID_PRODUCT = C.ID_PRODUCT
    LEFT JOIN klink_whm_testing.dbo.MASTER_WAREHOUSE D ON B.ID_WAREHOUSE = D.ID_WAREHOUSE
    WHERE A.ID_PRODUCT = @productId AND B.CATEGORY = 0 AND B.ID_WAREHOUSE = @warehouseId AND B.DAMAGED = 0 AND C.STATUS = 0 AND C.IS_PUSAT = 0
...
exports.updateStock = (req, res) => {
  ...
  pool_whm.then(pool => {
    ...
    .input('rackId', sql.VarChar, req.params.id_rack)
    .query(`UPDATE klink_whm_testing.dbo.T_STOCK SET QTY = QTY - @qty  WHERE ID_RACK = $rackId`, (err, result) => {
      if (err) throw err
      res.json(result.recordset)
    })
  })
...
exports.updateQtySend = (req, res) => {
  ...
  pool_whm.then(pool => {
    ...
    .input('receiptNo', sql.VarChar, req.params.kw)
    .input('aliasId', sql.VarChar, req.params.produk_alias_id)
    .query(`UPDATE klink_whm_testing.dbo.T_SALESSIMULATION SET QTY_SEND = QTY_SEND + @qty, QTY_SISA = QTY_SISA - @qty 
    WHERE KWITANSI_NO = @receiptNo AND PRODUK_ALIAS_ID = @aliasId`, (err, result) => {
      if (err) throw err
      res.send({ message: 'Success update data' })
    })
  })
...
exports.checkStockProductPcs = (req, res) => {
  ...
  pool_whm.then(pool => {
    ...
    LEFT JOIN klink_whm_testing.dbo.MASTER_PRODUK C ON A.ID_PRODUCT = C.ID_PRODUCT
    LEFT JOIN klink_whm_testing.dbo.MASTER_WAREHOUSE D ON B.ID_WAREHOUSE = D.ID_WAREHOUSE
    WHERE A.ID_PRODUCT = @productId AND B.CATEGORY = 1 AND B.ID_WAREHOUSE = @warehouseId AND B.DAMAGED = 0 AND C.STATUS = 0 AND C.IS_PUSAT = 0
    ORDER BY EXP_GUDANG`, (err, result) => {
      if (err) throw err
      res.json(result.recordset)
    })
...
exports.checkProductIndent = (req, res) => {
  ...
  pool_whm.then(pool => {
    ...
    .input('receiptNo', sql.VarChar, req.params.kw)
    .input('aliasId', sql.VarChar, req.params.produk_alias_id)
    .query(`SELECT QTY_SISA FROM klink_whm_testing.dbo.T_SALESSIMULATION WHERE KWITANSI_NO = @receiptNo AND PRODUK_ALIAS_ID = @aliasId AND  IS_INDENT = 0`, 
    (err, result) => {
      if (err) throw err
      res.json(result.recordset)
    })
...
exports.addFlagIndentSales = (req, res) => {
  ...
  pool_whm.then(pool => {
    pool.request()
    .input('no_do', sql.VarChar, req.params.no_do)
    .input('aliasId', sql.VarChar, req.params.produk_alias_id)
    .input('receiptNo', sql.VarChar, req.params.kw)
    .query(`UPDATE klink_whm_testing.dbo.T_SALESSIMULATION SET IS_ACTIVE = 0, IS_INDENT = 1, NO_DO = @no_do  WHERE PRODUK_ALIAS_ID = @aliasId AND KWITANSI_NO = @receiptNo`, 
    (err, result) => {
      if (err) throw err
      res.json(result.recordset)
    })
...
exports.updateBatchStock = (req, res) => {
  ...
  pool_whm.then(pool => {
    pool.request()
    .query(`UPDATE klink_whm_testing.dbo.T_STOCK SET EXP_PABRIK = NULL, EXP_GUDANG = NULL, EXP_BATCH = NULL, NO_BATCH = NULL WHERE QTY = 0 AND EXP_GUDANG IS NOT NULL`, 
    (err, result) => {
      if (err) throw err
      res.json(result.recordset)
    })
